...
void preorder_cluster(const Bvh &bvh, size_t nodes_per_cluster,
    ...
    if (!bvh.nodes[curr_node_idx].is_leaf()) {
        ...
        preorder_cluster(bvh, nodes_per_cluster, left_idx, counter, node_idx_to_cluster_idx);
        preorder_cluster(bvh, nodes_per_cluster, right_idx, counter, node_idx_to_cluster_idx);
    } else {
        node_idx_to_cluster_idx[curr_node_idx] = counter / nodes_per_cluster;
        counter++;
    }
